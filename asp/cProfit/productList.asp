...
if len(serviceName) > 0 then 
	if len(predicate) > 0 then predicate = predicate & "and " end if
	predicate = predicate & "[service] = '" & serviceName & "' " 
end if
...
SQL = "select " &_
		...
		"group by a.[product code], a.[product description] " &_
			...
			<div class="mdl-grid">
				...
				<div class="mdl-cell mdl-cell--12-col mdl-shadow--2dp" style="padding: 15px;">
					...
					<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="display: block; text-align: center; background-color: rgb(228, 233, 244); width: 100%">
						...
						%>
						Branch: <% =branchTitle %>;&nbsp;&nbsp;Officer: <% =officerTitle %>;&nbsp;&nbsp;Service Class: <% =groupTitle %>;&nbsp;&nbsp;Service: <% =serviceTitle %>
					</div>
					...
					if not rs.eof then 
						...
						<table align="center" class="cNoteTable sortable">
							<thead>
								<tr>
									<th class="alignLeft cNoteFixedCellMedium"><div>Product Code</div></th>
									<th class="alignLeft cNoteFixedCellHuge"><div>Product Description</div></th>
							...
							<tbody style="height: 550px;">
								...
								next 	
									%>
									<tr id="<% =rs("product code") %>" class="cNoteRow" 
										onclick="window.location.href='accountList.asp?id=<% =customerID %>&officer=<% =officerName %>&group=<% =groupName%>&product=<% =rs("product code") %>&service=<% =serviceName %>&branch=<% =branchName %>'"> 
										
										<td class="alignLeft cNoteFixedCellMedium"><div><% =rs("product code") %></div></td>
										<td class="alignLeft cNoteFixedCellHuge"><div><% =rs("product description") %></div></td>
							...
							<tfoot>
									<tr> 
										...
										<td class="alignRight" style="width: 291px;"><div>Totals:</div></td>
										<td class="alignRight cNoteFixedCellSmall cNoteMoney"><div><% =formatNumber(totalsArray(2),0) %></div></td>
										...
										<td class="alignRight cNoteFixedCellLarge cNoteMoney"><div><% =formatCurrency(totalsArray(10),0) %></div></td>
