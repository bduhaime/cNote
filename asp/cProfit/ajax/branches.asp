...
if isEmpty(customerID) then 
	...
	if not isNumeric(customerID) then 
		...
		customerPredicate = "AND customerID = " & customerID & " " 
	end if
end if 
...
SQL = "select " &_
		...
		"from ( " &_
			...
			"and [Branch Description] not in ('Treasury','Manually Added Accounts') " &_
...
while not rsOff.eof
	...
	json = json & "{"
	json = json & """DT_RowId"":""" & rsOff("branch description") & ""","
	json = json & """branchDescription"":""" & rsOff("branch description") & ""","
