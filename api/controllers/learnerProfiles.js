...
module.exports.set = function( https ) {
  ...
  //====================================================================================
  https.get( '/api/learnerProfiles', utilities.jwtVerify, async ( req, res ) => {
  //====================================================================================
	 ...
	 try {
		...
		const SQL = `
			select
				id,
				name
			from events.dbo.learnerProfiles
		`;
		...
		logger.log({ level: 'error', label: 'GET:api/learnerProfiles', message: err, user: req.session.userID });
