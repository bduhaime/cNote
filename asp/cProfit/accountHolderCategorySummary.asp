...
entityName = request.querystring("name") 

select case selectedSummary
	case "Branch"
		queryPredicate = "[Branch Description] = '" & entityName & "' "
	case "Officer"
		queryPredicate = "[Officer Name] = '" & entityName & "' "
	case else 
		queryPredicate = "[Officer Name] = '" & entityName & "' "
end select 
...
entitySQL = "select " &_
			"[Account Holder Name] = isNull([Account Holder Name], '" & metricSummaryRowTitle & "'), " &_
			strSumColumns & ", " &_
			strRowTotal & " " &_
		"from " &_
			"( " &_
		...
		") as y " &_
		"group by rollup([Account Holder Number]) " 
		
dbug(entitySQL)

entitySummary = jsonDataTable(entitySQL)
...
<head>
	...
	<script type="text/javascript">
		...
		function drawVisualization() {
		   ...
		   var dataTable = new google.visualization.DataTable(<% =entitySummary %>);
		   ...
		   var officerCategorySummaryTable = new google.visualization.Table(document.getElementById('dataTableDiv'));
			...
			google.visualization.events.addListener(officerCategorySummaryTable, 'select', function() {
				...
				if(selectedItem) {					
					...
					window.location.href = "customerProfit_officerAccountCategorySummary.asp?name=" + selectedEntityName 
																				+ "&id=" + <% =customerID %>
																				+ "&class=" + selectedClass 
			...
			<div class="mdl-grid">
				...
				<div class="mdl-cell mdl-cell--6-col mdl-shadow--2dp" style="padding: 15px;">
					...
					<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="display: block; text-align: center; background-color: rgb(228, 233, 244); width: 100%">
						<h9><b><% =selectedSummary %>&nbsp;<% =entityName %>&nbsp;Overview</b></h9>
					</div>
